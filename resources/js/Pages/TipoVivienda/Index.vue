...
const queryList = async (consulta) => {
  datas.isLoad = true
  const url = route('tipovivienda.query', { query: consulta.toUpperCase() })
  await axios
    .post(url)
    .then((response) => {
      if (response.data.isSuccess) {
...
const destroyData = async (id) => {
  const url = route('tipovivienda.destroy', id)
  await axios
    .delete(url)
    .then((response) => {
...
<template>
  <AppLayout title="Tipo Vivienda">
    ...
    >
      <!-- Header -->
      <HeaderIndex :title="'Tipo de Vivienda'">
        <template #link>
          <a
            v-if="
              $page.props.user.roles.includes('super-admin') ||
              $page.props.user.permissions.includes('TIPO_VIVIENDA.CREAR') ||
              $page.props.user.permissions.includes('TIPO_VIVIENDA.CREAR')
            "
            class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none"
            :href="route('tipovivienda.create')"
          >
            <svg
                  ...
                  tipodocumento.updated_at != null
                    ...
                    <td class="px-3 py-3">
                      <Link
                        v-if="
                          $page.props.user.roles.includes('super-admin') ||
                          $page.props.user.permissions.includes('TIPO_VIVIENDA.EDITAR') ||
                          $page.props.user.permissions_roles.includes('TIPO_VIVIENDA.EDITAR')
                        "
                        :href="route('tipovivienda.edit', item.id)"
                      ...
                      <button
                        ...
                        v-if="
                          $page.props.user.roles.includes('super-admin') ||
                          $page.props.user.permissions.includes('TIPO_VIVIENDA.ELIMINAR') ||
                          $page.props.user.permissions_roles.includes('TIPO_VIVIENDA.ELIMINAR')
                        "
