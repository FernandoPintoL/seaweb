...
const queryList = async (consulta) => {
    datas.isLoad = true
    const url = route('visitante.query', { query: consulta.toUpperCase() })
    await axios
        .post(url)
        .then((response) => {
            if (response.data.isSuccess) {
...
const queryListSaltoTake = async (consulta, skip, take) => {
    datas.isLoad = true
    const url = route('visitante.query', {
...
const destroyData = async (id) => {
    const url = route('visitante.destroy', id)
    await axios
        .delete(url)
        .then((response) => {
                                ...
                                <template #tbl-body>
                                    <tr v-for="item in datas.list" :key="item.id">
                                        ...
                                        <td v-if="props.editar || props.eliminar" class="px-3 py-3">
                                            <Link v-if="props.editar" :href="route('visitante.edit', item.id)"
