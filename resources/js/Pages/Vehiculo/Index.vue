...
const queryList = async (consulta) => {
  datas.isLoad = true
  const url = route('vehiculo.query', { query: consulta.toUpperCase() })
  await axios
    .post(url)
    .then((response) => {
      if (response.data.isSuccess) {
...
const queryListSaltoTake = async (consulta, skip, take) => {
  datas.isLoad = true
  const url = route('vehiculo.query', {
...
const destroyData = async (id) => {
  const url = route('vehiculo.destroy', id)
  await axios
    .delete(url)
    .then((response) => {
                    ...
                    >
                      {{
                        item.tipo_vehiculo == null
                          ? ''
                          : item.tipo_vehiculo.toUpperCase()
                      }}
                    </td>
                    <td
                      class="px-3 py-3 text-sm text-gray-500 whitespace-nowrap dark:text-white"
