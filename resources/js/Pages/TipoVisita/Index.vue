...
const queryList = async (consulta) => {
    datas.isLoad = true
    const url = route('tipovisita.query', { query: consulta.toUpperCase() })
    await axios
        .post(url)
        .then((response) => {
            if (response.data.isSuccess) {
...
const destroyData = async (id) => {
    const url = route('tipovisita.destroy', id)
    await axios
        .delete(url)
        .then((response) => {
                            ...
                            <TableGrl>
                                <template #tbl-header>
                                    <tr>
                                        ...
                                        <th v-if="props.editar || props.eliminar" class="px-6 py-3 text-start">
                                            <div class="flex items-center gap-x-2">
                                                <span
                                                    class="text-xs font-semibold uppercase tracking-wide text-gray-800 dark:text-neutral-200">
                                                    _
                                ...
                                <template #tbl-body>
                                    <tr v-for="item in datas.list" :key="item.id">
                                        ...
                                        <td v-if="props.editar || props.eliminar" class="px-3 py-3">
                                            <Link v-if="props.editar" :href="route('tipovisita.edit', item.id)"
